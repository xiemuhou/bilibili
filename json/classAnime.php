...
class bilibiliAnime
{
    ...
    {
        $url = "https://api.bilibili.com/x/space/bangumi/follow/list?type=1&follow_status=0&pn=1&ps=12&vmid=$uid";    //修改记录，15→12
        $info = json_decode(file_get_contents($url), true);
        $total = $info['data']['total'];    //修改记录，新增
    ...
    public function __construct($uid, $cookie)
    {
        ...
        for ($i = 1; $i <= ceil($this->total / 12); $i++) {    //修改记录，15→12
            //复制url浏览器打开可以看B站api数据格式
            $url = "https://api.bilibili.com/x/space/bangumi/follow/list?type=1&follow_status=0&pn=$i&ps=12&vmid=$uid";    //修改记录，15→12
            ...
            foreach ($info['data']['list'] as $data) {
                ...
                //修改记录，新番没有title（没有不等于null），校验会报错
                if (isset($data['new_ep']['title'])) {
                    array_push($this->fan_number, $this->fan_number($data['new_ep']['title']));
                }else{
                    array_push($this->fan_number, NULL);
                }
